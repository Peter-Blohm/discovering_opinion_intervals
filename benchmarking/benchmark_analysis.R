...
files <- list.files(path = "logs2/epinions", pattern = "\\.csv$", full.names = TRUE)
...
filled_data <- full_slashdot_data %>%
    ...
    filter(row_number()==1) %>%
    group_by(algorithm, num_clusters, type, seed, chunks) %>%
    complete(runtime = seq(0, 120000, by=100)) %>%
    arrange(runtime) %>%
...
g <- ggplot(summary_df %>% mutate(algorithm = factor(algorithm,levels=c("gaia","venus"),labels=c("Gaia","Venus"))), aes(x=runtime,y=mean_obj,color = algorithm, group=interaction(num_clusters,algorithm))) +
  geom_line(aes(linetype=as.factor(num_clusters)),linewidth=1.5)+
  geom_ribbon(aes(ymin = mean_obj - std_obj/2,max = mean_obj + std_obj/2,fill=algorithm,color=NULL,group=interaction(num_clusters,algorithm)),alpha=0)+
  coord_cartesian(ylim=c(30000,40000)) + scale_color_manual(values=c("Gaia"="#00BFC4","Venus"="#F8766D")) +
  xlim(0,120000) +theme + labs(x="Runtime",y="Avg Disagreement")


ggsave(filename = "epinions_time_plot.pdf",
       ...
       width    = 8,                 # in inches (adjust to taste)
       height   = 6,                  # in inches
       units    = "in")
...
ggsave(filename = "epinions_conv_plot.pdf",
       plot     = g,        # or explicitly your ggplot object3000F", "#FFED00", "#151518", "#009EE0")) +
       device   = cairo_pdf,          # better text handling & UTF-8 supporttheme + scale_linewidth_identity() +guides()
       width    = 8,                 # in inches (adjust to taste)
       height   = 6,                  # in inches
