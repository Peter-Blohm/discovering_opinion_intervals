...
THREADS=61
SOLUTION_OUT_FOLDER="benchmarking/synthetic_solutions_2"
LOG_OUT_FOLDER="benchmarking/synthetic_logs_2"
CONFIG_FOLDER="benchmarking/synthetic_configs/*"
STRUCT_FOLDER="benchmarking/synthetic_structs/*"
INSTANCE_FOLDER="/Data/Interval_Development/Synthetic_Datasets_2/*.json"
...
#        STRUCT_FILE=$(printf "../data/%s.json" "$STRUCT")
        ...
        {
          "$RUST_EXE" "$INSTANCE" "$STRUCT" "$CONFIG" "$SOLUTION_FILE" gaic --seed "$SEED" > "$LOG_FILE" ; echo "${INSTANCE##*/}", "${CONFIG##*/}", "${STRUCT##*/}", "$SEED", "$(tail $LOG_FILE -n 1)"
        } &
