#!/bin/bash
# Assumes you're in /Data/Interval_Development/ScalableCorrelationClustering/benchmarking

GS=$(seq 1 50)
THREADS=51
# SOLUTION_BASE_FOLDER="/Data/Interval_Development/ScalableCorrelationClustering/benchmarking/solutions"
LOG_OUT_FOLDER="/Data/Interval_Development/ScalableCorrelationClustering/benchmarking/logs"
# CONFIG_FOLDER="benchmarking/configs/*"
# STRUCT_FOLDER="benchmarking/structs/*"
INSTANCE_FOLDER="/Data/Interval_Development/Datasets/*.graph"
CPP_EXE="/Data/Interval_Development/ScalableCorrelationClustering/build/signed_graph_clustering_evolutionary"
# echo "instance, seed, energy, runtime"
# Loop through each combination of parameters
for SEED in $GS;
do
  for INSTANCE in $INSTANCE_FOLDER; 
  do
    LOG_FILE=$(printf "%s/%s_%s.csv" "$LOG_OUT_FOLDER" "${INSTANCE##*/}" "$SEED")

    {
      mpirun -n 1 "$CPP_EXE" "$INSTANCE" --seed "$SEED" --time_limit 180 > "$LOG_FILE" 2>/dev/null
    } &
    # Wait for the maximum number of parallel jobs to be reached
    while [ "$(jobs | wc -l)" -ge $THREADS ]; do
      # echo "Waiting for jobs to finish..."
      sleep 1
    done
  done
done
wait
