...
use algorithms::{
    brute_force_interval_structure, cc_compute_violations, cc_local_search,
    greedy_additive_edge_contraction,
};
use data_types::{IntervalStructure, SignedGraph};
use gaic::greedy_absolute_interval_contraction;

fn main() {
    let (graph, interval_structure, config, output_filename, algorithm, run_cc_local_search, seed) =
        parse_args();
    ...
    if algorithm == "gaic" {
        let node_labels = greedy_absolute_interval_contraction(
            &signed_graph,
            &interval_structure,
            &config,
            seed as u64,
        ...
        // GAEC
        let node_labels = greedy_additive_edge_contraction(
            signed_graph.num_vertices,
            &signed_graph.edges,
            num_clusters,
        );
        ...
        let (_, interval_violations) =
            brute_force_interval_structure(&signed_graph.edges, &node_labels, &interval_structure);
        ...
        if run_cc_local_search {
            ...
            let local_search_violations =
                cc_compute_violations(&signed_graph.edges, &local_search_node_labels);
...
fn parse_args() -> (
    ...
    if args.len() < 5 {
        println!(
            "Usage: {} <graph_file> <interval_file> <config_file> <output_file> <algorithm> [opts]",
            args[0]
        );
        println!(
            "Algorithms: gaec (Greedy Additive Edge Contraction), gaic (Greedy Absolute Interval Contraction)"
        );
    ...
    let graph: SignedGraph = serde_json::from_str(&graph_json_data).unwrap_or_else(|_| {
        panic!(
            "Failed to parse graph JSON from {}",
    ...
    let interval_structure: IntervalStructure = serde_json::from_str(&interval_json_data)
        .unwrap_or_else(|_| {
            panic!(
                "Failed to parse interval JSON from {}",
                interval_filename.display()
            )
        });
    ...
    let config_structure: GaicConfig =
        serde_json::from_str(&config_json_data).unwrap_or_else(|_| {
            panic!(
